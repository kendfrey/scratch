(function (factory) {
    ...
    class Context {
        constructor() {
            ...
            this.segmentation = 256;
            /** Gets or sets the default properties for all strokes. */
            this.default = { x: 0, y: 0, r: 0, g: 0, b: 0, a: 1, w: 1 };
        }
        /**
    ...
    class Stroke {
        ...
        bezier(...points) {
            ...
            return Stroke._create(t => {
                ...
                for (const prop of props) {
                    result[prop] = bezier(t, points.map(p => p[prop]));
                }
         ...
         */
        ellipse(x, y, rx, ry, start = 0, ccw = false, form = "parametric") {
         ...
         * @param ccw True to reverse the direction of rotation to counter-clockwise.
         */
        circle(x, y, r, start = 0, ccw = false) {
        ...
        rotate(angle, x = 0, y = 0) {
            return this.matrix(Math.cos(angle), Math.sin(angle), -Math.sin(angle), Math.cos(angle), -Math.cos(angle) * x + Math.sin(angle) * y + x, -Math.sin(angle) * x + -Math.cos(angle) * y + y);
        }
        /**
     ...
     */
    function drawLine(data, start, end) {
     ...
     */
    function drawLinePixel(data, x, y, start, end, steep, reverse) {
     ...
     */
    function drawPixel(data, x, y, r, g, b, a, blend) {
